...
export default function Reply({
  ...
  const canEdit =
    isAuthor &&
    new Date().getTime() - new Date(reply.createdAt).getTime() < 15 * 60 * 1000;
  const canDelete = isAuthor && !reply.isDeleted;
  const canRestore =
    isAuthor &&
    reply.isDeleted &&
    new Date().getTime() - new Date(reply.deletedAt).getTime() < 15 * 60 * 1000;
  ...
  return (
    <div
      className={`p-4 border rounded-md ${
        reply.isDeleted ? "bg-red-50 border-red-200" : "bg-white"
      }`}
    >
      ...
      <p
        className={`text-slate-700 ${
          reply.isDeleted ? "italic text-slate-500" : ""
        }`}
      >
